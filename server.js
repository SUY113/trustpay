...
app.post("/enroll-admin", async (req, res) => {
  ...
  try {
    const cryptoConfigPath = path.resolve(
      __dirname,
      "..",
      "..",
      "first-network",
      "crypto-config",
      "peerOrganizations",
      `org${orgName}.example.com`,
      "users",
      `Admin@org${orgName}.example.com`,
      "msp"
    );
    const certPath = path.join(
      cryptoConfigPath,
      "signcerts",
      `Admin@org${orgName}.example.com-cert.pem`
    );
    const keyPath = path.join(
      cryptoConfigPath,
      "keystore",
      fs.readdirSync(path.join(cryptoConfigPath, "keystore"))[0]
    );
    ...
    res
      .status(200)
      .json({
        message: 'Successfully imported admin user "Admin" into the wallet',
      });
  } catch (error) {
    console.error(`Failed to import admin user "Admin": ${error}`);
    res
      .status(500)
      .json({ error: `Failed to import admin user "Admin": ${error.message}` });
  }
});
...
app.post("/register-user", async (req, res) => {
  ...
  try {
    const cryptoConfigPath = path.resolve(
      ...
      `org${orgName}.example.com`,
      "users",
      `${userName}@org${orgName}.example.com`,
      "msp"
    );
    ...
    if (!fs.existsSync(cryptoConfigPath)) {
      return res
        .status(400)
        .json({
          error: `User "${userName}" in organization "org${orgName}" does not exist`,
        });
    }
    ...
    const certPath = path.join(
      cryptoConfigPath,
      "signcerts",
      `${userName}@org${orgName}.example.com-cert.pem`
    );
    const keyPath = path.join(
      cryptoConfigPath,
      "keystore",
    ...
    if (userExists) {
      return res
        .status(400)
        .json({
          error: `An identity for the user "${userName}" already exists in the wallet`,
        });
    }
    ...
    res
      .status(200)
      .json({
        message: `Successfully imported user "${userName}" into the wallet`,
      });
  } catch (error) {
    console.error(`Failed to import user "${userName}": ${error}`);
    res
      .status(500)
      .json({ error: `Failed to import user "${userName}": ${error.message}` });
  }
});
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const beginIndex =
      userCertificate.indexOf("-----BEGIN CERTIFICATE-----") +
    ...
    res
      .status(500)
      .json({ error: `Failed to login user "${userName}": ${error.message}` });
  }
});
//DATABASE
app.post("/input-info", async (req, res) => {
  ...
  if (!userName || !orgName || !name || !age || !ethAddress) {
    return res
      ...
      .json({
        error:
          "Please provide all required fields: userName, orgName, name, age, ethAddress",
      });
  }
  ...
  try {
    ...
    if (!userExists) {
      return res
        .status(400)
        .json({
          error: `An identity for the user "${userName}" does not exist in the wallet. Run the registerUser.js application before retrying`,
        });
    }
